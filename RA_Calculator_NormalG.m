...
for i = 1:length(Try_folder)
    ...
    for j = 1:length(Try_lst)
        ...
        %calcul de l'amplitude et de la déviation latérale du mouvement
        Y_position = Try.T(:,18);
        ...
        for l = 1:length(Z);
            Arm_length = [Arm_length
                sqrt(((abs(Try.T(l,2)-Try.T(l,17)))^2)+((abs(Try.T(l,3)-Try.T(l,18)))^2)+((abs(Try.T(l,4)-Try.T(l,19)))^2))];
        end
        ...
        Z_start = [mean(Try.T(1:10,17)) mean(Try.T(1:10,18)) mean(Try.T(1:10,19))];
        Z_end = [mean(Try.T(length(Z)-10:length(Z),17)) mean(Try.T(length(Z)-10:length(Z),18)) mean(Try.T(length(Z)-10:length(Z),19))];
        ...
        TargetUp = [mean(Try.T(:,20)) mean(Try.T(:,21)) mean(Try.T(:,22))];   
        TargetDown = [mean(Try.T(:,23)) mean(Try.T(:,24)) mean(Try.T(:,25))];
        ...
        Vertical_Amp_Offset_Onset = abs(Z(offset,1)-Z(onset,1));
